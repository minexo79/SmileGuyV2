...
class Sokoban():
    ...
    def userinput(self,x):
        y={"🔼":self.up,"🔽":self.down,"◀":self.letf,"▶":self.right}
        y[x]()

    def up(self):
        if self.map1[self.player[0]-1][self.player[1]] != "🔲" :
            #自己移動
            if ([self.player[0]-1,self.player[1]] !=self.end) and ([self.player[0]-1,self.player[1]] not in self.box):
                self.player[0]-=1
            #推箱子
            elif ([self.player[0]-1,self.player[1]] in self.box) :
    ...
    def letf(self):
        if self.map1[self.player[0]][self.player[1]-1] != "🔲" :
            #自己移動
            if ([self.player[0],self.player[1]-1] !=self.end) and ([self.player[0],self.player[1]-1] not in self.box):
                self.player[1]-=1
            #推箱子
            elif ([self.player[0],self.player[1]-1] in self.box) :
    ...
    def mapprint(self):
        ...
        # 人物
        map2[self.player[0]][self.player[1]]="🌝"
        # 終點
        map2[self.end[0]][self.end[1]]="🟨"
        for i in self.box :
            #箱子
            map2[i[0]][i[1]]="🔳"
